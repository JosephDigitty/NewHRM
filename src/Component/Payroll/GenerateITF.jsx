...
const GenerateITF = () => {
  ...
  const [ITFPeriod, setITFPeriod] = useState([]);
  const [ITFByPeriod, setITFByPeriod] = useState([]);
  ...
  useEffect(() => {
    const getITF = async () => {
      try {
        setLoading(true);
        const response = await api.get("employee/itf/all");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        const res = response.data.filteredITF.map((pay) => ({
          EmployeeName: pay.employeeId.userId.fullname,
          Department: pay.employeeId.job?.department?.department_Name,
          Grade: pay.employeeId.job?.grade?.gradeName,
          Period: pay.period,
        ...
        setLoading(false);
        setITFPeriod(response.data.filteredITF);
      } else {
        showError(response.data.error);
        setLoading(false);
      }
  ...
  const sumITF = ITFPeriod.reduce((acc, item) => acc + item.amount, 0);
  const ITFInNaira = formatCurrency(sumITF);
  const isITF = ITF.length > 0;
     const downloadExcel = () => {
    // decide which data to export
    const dataToExport =
      ITFByPeriod.length > 0 ? ITFByPeriod : ITF;
  ...
  return (
    <LoadingState loading={loading} loadingText="Loading ITF data...">
      <div className="px-3 flex justify-between">
        <div>
          ...
          <p className="text-sm text-black/80 font-thin pb-5 font-sans">
            View and manage ITF Statutory payment for your Organisation
          </p>
        </div>
      ...
      {ITFByPeriod && ITFByPeriod.length > 0 ? (
        ...
        <div className="pl-3 mr-10 pr-10">
          <DataTable columns={allPensionCollumn} data={ITF} pagination />
        </div>
      ) : (
        <div>loading...</div>
      )}
