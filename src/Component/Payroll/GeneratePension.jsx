...
const GeneratePension = () => {
  ...
  const [pensionPeriod, setPensionPeriod] = useState([]);
  const [pensionByPeriod, setpensionByPeriod] = useState([]);
  ...
  useEffect(() => {
    const getPension = async () => {
      try {
        setLoading(true);
        const response = await api.get("/employee/pension/all",);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post(
        "/employee/pension/period/",
        { payDate },
      );
      if (response.data.success) {
        const res = response.data.filteredPayroll.map((pay) => ({
          EmployeeName: pay.employeeId.userId.fullname,
          Department: pay.employeeId.job?.department?.department_Name,
          Grade: pay.employeeId.job.grade.gradeName,
          Period: pay.period,
        ...
        setpensionByPeriod(res);
        showSuccess("payroll Successful");

        setLoading(false);
        setPensionPeriod(response.data.filteredPayroll);
      } else {
        showError(response.data.error);
        setLoading(false);
      }
  ...
  const sumPension = pensionPeriod.reduce((acc, item) => acc + item.amount, 0);
  const pensionInNaira = formatCurrency(sumPension);
  const isPension = pension.length > 0;

    const downloadExcel = () => {
    // decide which data to export
    const dataToExport =
      pensionByPeriod.length > 0 ? pensionByPeriod : pension;
    ...
    saveAs(blob, `Pension_${new Date().toISOString()}.xlsx`);
  };

  return (
    <LoadingState loading={loading} loadingText="Loading Pension data...">
      <div className="px-3 flex justify-between">
        <div>
          ...
          <p className="text-sm text-black/80 font-thin pb-5 font-sans">
            View and manage pension contribution for your employee
          </p>
        </div>
      ...
      {pensionByPeriod && pensionByPeriod.length > 0 ? (
        <div className="pl-3 mr-10 pr-10">
          <p className="font-thin text-lg text-black font-sans pb-3">
            {`Total pension for the period is ${pensionInNaira}`}
          </p>
          <DataTable
            columns={allPensionCollumn}
            data={pensionByPeriod}
            pagination
          />
        </div>
      ) : isPension ? (
        <div className="pl-3 mr-10 pr-10">
          <DataTable columns={allPensionCollumn} data={pension} pagination />
        </div>
      ) : (
        <div>loading...</div>
      )}
...
export default GeneratePension;
